...
void scheduler(Job jobs[], int count) {       // Loop through each job
    for(int i = 0; i < count; i++) {          // Print message indicating the start of a job's execution
...
int main() {        // Define and initialize an array of jobs with the given JOB_COUNT and DURATION
    Job jobs[JOB_COUNT] = {
        {1, DURATION},
        {2, DURATION},
        {3, DURATION}
